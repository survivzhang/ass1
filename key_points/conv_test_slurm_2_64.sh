...
#SBATCH --job-name=conv_test_2_64
#SBATCH --output=conv_test_2_64.out
#SBATCH --error=conv_test_2_64.err
...
echo "=== 2D Convolution Performance Test (Threads 2-64) ==="
...
echo ""
echo "=== Thread Performance Analysis (2-64 threads) ==="

# Test thread counts from 2 to 64
for threads in $(seq 2 64); do
    ...
    output=$(./conv_test -H $MATRIX_SIZE -W $MATRIX_SIZE -h $KERNEL_SIZE -w $KERNEL_SIZE \
        -f input_test_2_64.txt -g kernel_test_2_64.txt -o output_test_2_64.txt -p 2>&1)
...
echo ""
echo "=== Performance Analysis Summary (Threads 2-64) ==="
echo "Matrix Size: ${MATRIX_SIZE}x${MATRIX_SIZE}"
echo "Kernel Size: ${KERNEL_SIZE}x${KERNEL_SIZE}"
echo "Thread Range: 2-64"
...
# Find optimal thread count in 2-64 range
echo ""
echo "=== Optimization Analysis (Threads 2-64) ==="
optimal_threads=2
best_speedup=0.0
...
echo "Optimal thread count for maximum speedup (2-64): $optimal_threads threads (${best_speedup}x speedup)"
echo "Best thread count with >80% efficiency (2-64): $best_efficiency_threads threads"
echo "Best computing time (2-64): ${computing_times[$optimal_threads]}s"

# Send output file as email attachment
echo ""
echo "Sending results (2-64 threads) via email..."
mail -s "Convolution Performance Test Results 2-64 Threads - Job $SLURM_JOB_ID" -a conv_test_2_64.out 24070858@student.uwa.edu.au < /dev/null

echo ""
echo "Thread range 2-64 test completed successfully!"
