...
#SBATCH --job-name=conv_test_11_32
#SBATCH --output=conv_test_11_32.out
#SBATCH --error=conv_test_11_32.err
...
echo "=== 2D Convolution Performance Test (Threads 11-32) ==="
...
echo ""
echo "=== Thread Performance Analysis (11-32 threads) ==="

# Test thread counts from 11 to 32
for threads in $(seq 11 32); do
    ...
    output=$(./conv_test -H $MATRIX_SIZE -W $MATRIX_SIZE -h $KERNEL_SIZE -w $KERNEL_SIZE \
        -f input_test_11_32.txt -g kernel_test_11_32.txt -o output_test_11_32.txt -p 2>&1)
...
echo ""
echo "=== Performance Analysis Summary (Threads 11-32) ==="
echo "Matrix Size: ${MATRIX_SIZE}x${MATRIX_SIZE}"
echo "Kernel Size: ${KERNEL_SIZE}x${KERNEL_SIZE}"
echo "Thread Range: 11-32"
...
# Find optimal thread count in 11-32 range
echo ""
echo "=== Optimization Analysis (Threads 11-32) ==="
optimal_threads=11
best_speedup=0.0
...
echo "Optimal thread count for maximum speedup (11-32): $optimal_threads threads (${best_speedup}x speedup)"
echo "Best thread count with >80% efficiency (11-32): $best_efficiency_threads threads"
echo "Best computing time (11-32): ${computing_times[$optimal_threads]}s"

# Send output file as email attachment
echo ""
echo "Sending results (11-32 threads) via email..."
mail -s "Convolution Performance Test Results 11-32 Threads - Job $SLURM_JOB_ID" -a conv_test_11_32.out 24070858@student.uwa.edu.au < /dev/null

echo ""
echo "Thread range 11-32 test completed successfully!"
